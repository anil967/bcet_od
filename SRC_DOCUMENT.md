...
3. **Production Server**:
   ...
   npm start
   ```
   The production server in `server.js` serves the built assets and handles
   `/api/register`, `/api/health`, and the admin API. Deploy the project as a
   Node.js application with the `start` command; do not deploy only the
   `dist/` folder to a static-only host, otherwise `/api/register` will return
   an HTML 404 page instead of JSON.
4. **Environment**: Configure `MONGODB_URI`, `MONGODB_DB`, `MONGODB_COLLECTION`,
   `ADMIN_USERNAME`, and `ADMIN_PASSWORD_HASH` in the server environment.
5. **Direct Local Viewing**: Open `index.html` directly in any modern browser.
   Registration cannot work when the HTML is opened directly because no API
   server is running.
